import { processApiCall } from "./api/timestamps";
import { getAuthorFeed, getMostRecentPosts } from "./db/posts";
...
export const handler = async (req: Request): Promise<Response> => {
  ...
  await processApiCall({ req, url, path, method });

  if (method === "GET") {
    if (path === "/api/most-recent-records") {
      const limit = parseInt(url.searchParams.get("limit") ?? "50");
      const cursor = url.searchParams.get("cursor");
      const records = getMostRecentPosts(limit, cursor);
    ...
    if (path === "/api/recent-records-by-user") {
      ...
      let records = getAuthorFeed(user_did, limit, cursor);

      if (records.length === 0) {
        await backfill(user_did);
